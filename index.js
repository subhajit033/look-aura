  ...
  try {
    ...
    app.post('/order', verifyJWT, async (req, res) => {
      ...
      try {
        ...
        const getData = await Packages.findOne({
          _id: new ObjectId(req.body._id),
        });

        const options = {
          currency: 'INR',
          amount: parseInt(getData?.price) * 100,
          receipt: new ObjectId().toString(),
        };
        console.log(options);
      ...
      if (digest === req.body.signature) {
        ...
        const updatedDoc = {
          $set: {
            isPaid: true,
            coins:
              parseInt(req.body.packageCoins) + parseInt(req.body.currentCoins),
          },
        };
        const option = { upsert: true };
    ...
    app.put('/updateProduct', verifyJWT, async (req, res) => {
      ...
      if (result) {
        ...
        const option = { upsert: true };
        const userCoinsUpdate = await Users.updateOne(
    ...
    app.get('/allCart', verifyJWT, async (req, res) => {
      const result = await AllDesigns.find({
        $and: [{ isSold: true }, { buyerEmail: req.decoded.email }],
      }).toArray();
      res.send(result);
    });
  } finally {
